      ...
      {selectedEvent && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="relative bg-white p-8 rounded-lg shadow-lg max-w-lg w-full mx-4">
            <h3 className="text-2xl font-semibold text-gray-800 mb-4">{selectedEvent.title}</h3>
            <img
              src={selectedEvent.image}
              alt={selectedEvent.title}
              className="w-full h-48 object-cover rounded mb-4"
            />
            <p className="text-gray-700 mb-4"><strong>Date:</strong> {selectedEvent.date}</p>
            <p className="text-gray-700 mb-4">
              <strong>Category:</strong> {selectedEvent.category.charAt(0).toUpperCase() + selectedEvent.category.slice(1)}
            </p>
            <p className="text-gray-700"><strong>Description:</strong> {selectedEvent.description}</p>
            <button onClick={closeModal} className="absolute top-3 right-3 text-green-bk px-3 py-1 rounded-full text-3xl font-medium">
              <IoCloseSharp/>
            </button>
          </div>
        </div>
